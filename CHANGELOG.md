...
## v1.8 — Onglet Dépenses
- Nouvelle **barre de navigation** « Clients | Dépenses » (visible uniquement pour l'équipe, pas pour les livreurs).
- Onglet **Dépenses** : ajout, modification et suppression de dépenses (libellé, montant, catégorie, date, note), enregistrées dans Firebase (collection `depenses`).
- **Bilan de la période** : ventes (sans frais de livraison) − dépenses = bénéfice net, affiché en rouge s'il est négatif.
- Le filtre de période (mois / année / semaine) est désormais **partagé** entre les deux onglets.
- Retrait des animations dans le tableau clients (la liste ne réapparaît plus en fondu à chaque clic).
